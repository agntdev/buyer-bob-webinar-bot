...
import { _resetStoreForTests } from "../src/lib/store.js";
...
describe("dialog specs (the publish gate replays these)", () => {
  it("every tests/specs/*.json spec passes against the real bot", async () => {
    ...
    // Fresh durable store per suite so owner-settings mutations in one spec
    // don't leak into webinar title/copy assertions in another.
    await _resetStoreForTests();
    const suite = await runSpecs(async () => {
      // Per-spec bot is already fresh (session isolation). Durable store is
      // shared intentionally so multi-step registration → export works within
      // a single suite; specs that need isolation use distinct userIds.
